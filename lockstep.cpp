...
void lockstep_init()
{
  ...
  while (result == -1)
    result = connect(ls_Socket, (struct sockaddr *)&Address, sizeof(struct sockaddr));
  ...
  int optval = 1;
  setsockopt(ls_listenSocket,SOL_SOCKET,SO_REUSEADDR, (char*)&optval,sizeof(optval));
...
//  Wait until we have a connection
  ...
  while (ls_Socket == INVALID_SOCKET)
    ls_Socket = accept(ls_listenSocket,(struct sockaddr*)&Address,&nAddressSize);
