...
public:
  bx_x11_gui_c (CConfigurator * cfg) { myCfg = cfg; bx_keymap = new bx_keymap_c(cfg); };
...
static bool
test_alloc_colors (Colormap cmap, u32 n_tries) {
  XColor color;
  ...
  void
bx_x11_gui_c::specific_init(unsigned tilewidth, unsigned tileheight)
{
  unsigned i;
  ...
  XSetWMProperties(bx_x_display, win, &windowName, &iconName,
      NULL /*argv*/, 0/*argc*/, &size_hints, &wm_hints,
      &class_hints);
...
// In all those cases, setting the parameter value will get you here.
  void
bx_x11_gui_c::mouse_enabled_changed_specific (bool val)
{
...
{
  u8 *old_line, *new_line, *text_base;
  u8 cChar;
  ...
  u8 split_textrow, split_fontrows;
  bool forceUpdate = 0, split_screen;
...
void bx_x11_gui_c::graphics_tile_update(u8 *tile, unsigned x0, unsigned y0)
{
  ...
  XPutImage(bx_x_display, win, gc, ximage, 0, 0, x0, y0,
            x_tilesize, y_size);
}

  bx_svga_tileinfo_t *
bx_x11_gui_c::graphics_tile_info(bx_svga_tileinfo_t *info)
{
  if (!info) {
  ...
  u8 *
bx_x11_gui_c::graphics_tile_get(unsigned x0, unsigned y0,
                          unsigned *w, unsigned *h)
{
  ...
  void
bx_x11_gui_c::graphics_tile_update_in_place(unsigned x0, unsigned y0,
                                      unsigned w, unsigned h)
{
...
bx_x11_gui_c::palette_change(unsigned index, unsigned red, unsigned green, unsigned blue)
{
  ...
  void
bx_x11_gui_c::dimension_update(unsigned x, unsigned y, unsigned fheight, unsigned fwidth, unsigned bpp)
{
