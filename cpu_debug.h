...
#define DEBUG_XX							                        \
  ...
  sprintf(dbg_strptr,bListing?"%08" LL "x: ":"%016" LL "x",         \
                                                 state.current_pc); \
  dbg_strptr += strlen(dbg_strptr);					                \
  if (!bListing)							                        \
...
#define UNKNOWN2							                        \
  ...
  sprintf(dbg_strptr,"Unknown opcode: %02x.%02x   ", opcode,        \
                                                         function); \
  dbg_strptr += strlen(dbg_strptr);					                \
  handle_debug_string(dbg_string);					                \
  return 0;
...
#define PRE_MFPR(mnemonic)						                    \
  if (bDisassemble) {							                    \
    DEBUG_XX;								                        \
    sprintf(dbg_strptr,#mnemonic " r%d, %s", REG_1&31,              \
                                               IPR_NAME[function]);	\
    dbg_strptr += strlen(dbg_strptr);					            \
  }
...
#define PRE_MTPR(mnemonic)						                    \
  if (bDisassemble) {							                    \
    DEBUG_XX;								                        \
    sprintf(dbg_strptr,#mnemonic " r%d, %s", REG_2&31,              \
                                               IPR_NAME[function]);	\
    dbg_strptr += strlen(dbg_strptr);					            \
  }
...
#define PRE_R12_R3(mnemonic)						                \
  if (bDisassemble) {									            \
    ...
    dbg_strptr += strlen(dbg_strptr);					            \
    if (!bListing) {							                    \
      sprintf(dbg_strptr,": (%" LL "x,%" LL "x)",state.r[REG_1],    \
                                                              V_2);	\
      dbg_strptr += strlen(dbg_strptr);				                \
    }									                            \
  }
...
#define PRE_F12_F3(mnemonic)						                \
  if (bDisassemble) {									            \
    DEBUG_XX;								                        \
    sprintf(dbg_strptr,#mnemonic " f%d, f%d, f%d", FREG_1, FREG_2,  \
                                                           FREG_3);	\
    dbg_strptr += strlen(dbg_strptr);					            \
    if (!bListing) {							                    \
...
#define PRE_FMEM(mnemonic)						                    \
  if (bDisassemble) {							                    \
    DEBUG_XX;								                        \
    sprintf(dbg_strptr, #mnemonic " f%d, %04xH(r%d)", FREG_1,       \
                                           (u32)DISP_16, REG_2&31); \
    dbg_strptr += strlen(dbg_strptr);					            \
    if (!bListing) {							                    \
      sprintf(dbg_strptr,": (%" LL "x)", state.r[REG_2]);			\
